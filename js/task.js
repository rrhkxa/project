 ...
 */
$(function() {
	var baseUrl = "http://club.xcar.com.cn/ajax_xbb/";
	 ...
	 */
	var pageJson = {
		getmoreUrl: 'http://club.xcar.com.cn/task/getMore', //请求更多数据接口
		submitApplyUrl: 'http://club.xcar.com.cn/talent_task/talentApply', //验证达人接口
		detailGetMoreUrl: 'http://club.xcar.com.cn/task/getMoreDlists', //详情页面更多接口
		addFollowUrl: 'http://club.xcar.com.cn/task/addFocus', //任务详情页添加关注接口
		requireAppUrl: 'http://club.xcar.com.cn/talent_task/requireApp', //申请达人接口
					...
					if (data.status == 1) {
						...
						if (data.dlists.length > 0) {
							...
							$.each($(data.dlists), function(index, val) {
								...
								html += '<a target="_blank" href="http://www.xcar.com.cn/bbs/viewthread.php?tid=' + val.tid + '">'
									...
									$.ajax({
										...
										.done(function(data) {
											if (data.code == 1) {
												...
												window.location.href = ("http://www.xcar.com.cn/bbs/post.php?action=newthread&fid=" + dataFid + "&task_id=" + dataTask + "&big_task=yes&extra=page%3D1")
											} else {
												transfer_pop(".state_pop", ".graybg_pop") //对话框
												$(".graybg_pop").show();
												$(".state_pop").show();
